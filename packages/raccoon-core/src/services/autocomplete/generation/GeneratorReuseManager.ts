...
/**
 * Reuses a pending completion generator when the user keeps typing and the new
 * prefix is a continuation of the previous one, avoiding a redundant backend
 * request. Ported verbatim from continue-rac
...
export class GeneratorReuseManager {
  ...
  async *getGenerator(
    ...
    for await (let chunk of this.currentGenerator?.tee() ?? []) {
      if (token.aborted && abortController && !abortController.signal.aborted) {
