...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try: `lildax models` to list available models", // raccoon_change - rebrand model command hint
      "Or check your config (raccoon.json) provider/model names", // raccoon_change - rebrand config hint
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, Raccoon does not support MCP authentication yet.` // raccoon_change - rebrand MCP error
  }
  return undefined
}
