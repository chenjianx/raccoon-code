...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle("Raccoon") // raccoon_change - rebrand terminal title
      return
    }
    ...
    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`Raccoon | ${route.data.id}`) // raccoon_change - rebrand terminal title prefix
    }
  })
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to Raccoon v${result.data.version}. Please restart the application.`, // raccoon_change - rebrand update dialog
    )
